  ...
  export default{
    ...
    methods: {
      needShow(type, text) {
        ...
        if (this.selectType[0] === ALL) {
          return true;
        } else {
          return type === this.selectType[0];
        }
      }
    },
    mounted() {
      this.$root.$on('ratingtype.select', (type) => {
        this.selectType[0] = type;
        this.$nextTick(() => {
          this.scroll.refresh();
        });
      });
      this.$root.$on('content.toggle', (oContent) => {
  ...
  .ratings
    ...
    .overview
      ...
      .overview-left
        ...
        padding: 6px 0
        width: 127px
        border-right: 1px solid rgba(7, 177, 27, 0.1)
        text-align: center
        .score
          margin-bottom: 6px
          line-height: 28px
